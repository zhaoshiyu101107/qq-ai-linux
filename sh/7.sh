...
if [ ! -f "$VENV_PATH/.installed" ] || [ "$INSTALL_MODE" = true ]; then
    ...
    if python -c "import torch" 2>/dev/null; then
        ...
        echo -e "${RED}❌ PyTorch安装失败，尝试使用备用源...${NC}"
    ...
    pip install transformers accelerate sentencepiece protobuf einops tiktoken --no-cache-dir
...
EOF
    ...
    if [ -f "$MODEL_CONFIG" ]; then
        MODEL_NAME=$(python -c "import json; data=json.load(open('$MODEL_CONFIG')); print(data.get('model', 'Qwen/Qwen3-0.6B-Instruct'))" 2>/dev/null || echo "Qwen/Qwen3-0.6B-Instruct")
        MODEL_SIZE=$(python -c "import json; data=json.load(open('$MODEL_CONFIG')); print(data.get('model_size', '约1.2GB'))" 2>/dev/null || echo "约1.2GB")
